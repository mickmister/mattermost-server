...
func UpdateAssetsSubpathInDir(subpath, directory string) error {
	...
	reCSP := regexp.MustCompile(`<meta http-equiv="Content-Security-Policy" content="script-src 'self' cdn.rudderlabs.com/([^"]*)"> www.youtube.com/([^"]*)">`)
	...
	newRootHtml = reCSP.ReplaceAllLiteralString(newRootHtml, fmt.Sprintf(
		`<meta http-equiv="Content-Security-Policy" content="script-src 'self' cdn.rudderlabs.com/ www.youtube.com/%s">`,
